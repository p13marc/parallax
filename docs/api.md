...
`Event` (StreamStart/Segment/Tags/Eos/Error/CapsChanged/Gap | Seek | FlushStart/FlushStop/Pause/Resume/Custom), `SegmentEvent` (`to_running_time`/`to_stream_time`), `SeekEvent`/`SeekFlags`, `PipelineItem`, `TagList`/`TagValue`/`TagMergeMode`; `ClockTime`, `Clock`, `ClockFlags`, `ClockProvider`, `SystemClock`, `PipelineClock`.
