...
- **Output arenas are executor-sized** (#84): `OutputBudget` = `max(link capacity per src pad, summed across pads) + IN_FLIGHT_MARGIN`, delivered via the defaulted `set_output_budget` on `Element`/`Transform`/`AsyncElementDyn`. `max` not sum per pad — fan-out clones share one slot. Elements use `memory::OutputArena` (lazy build, `reset()` on resize, `admit()` for pre-encode admission control, `Error::PoolExhausted` when full) rather than a hand-rolled `Option<SharedArena>`. `memory::defaults` holds the slot counts and `MAX_OUTPUT_ARENA_BYTES` clamp.
