...
- **Two un-budgeted process-wide `static OnceLock<SharedArena>`s remain** — `metadata/klv.rs` and `demux/mpegts.rs`. One arena shared by every instance in the process, and neither owner implements an element trait, so `set_output_budget` cannot reach them: `create_klv_buffer`'s caller is the public `KlvEncoder::to_buffer(&self)`, and `FrameCollector` is reached through shared state inside mpeg2ts_reader's demultiplexer. #76 (demuxer-as-element) would settle the second. See #95; the other three are done. Two arenas are deliberately *not* `OutputArena` and say so in a comment: `IpcSink`'s (exists only to pass its fd via SCM_RIGHTS) and `Mp4MuxTransform::flush`'s (one slot, sized to the finished file).
