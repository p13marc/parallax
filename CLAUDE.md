...
- **Video dimensions in metadata**: two conventions coexist — `Metadata.format` (`MediaFormat::VideoRaw`, read by `EncoderElement`) and the legacy `"width"`/`"height"` `u64` custom keys. Use `Metadata::set_video_dims()` / `video_dims()`, which write and read **both**; updating only one leaves the other stale and silently mis-sizes frames downstream, and reading only one is how `AutoVideoSink` used to miss geometry a `VideoRaw` producer had set.
