...
- **One un-budgeted process-wide `static OnceLock<SharedArena>` remains** — `metadata/klv.rs`: `create_klv_buffer`'s caller is the public `KlvEncoder::to_buffer(&self)`, which implements no element trait, so `set_output_budget` cannot reach it (#95; the other four are done — `demux/mpegts.rs` went per-instance with #76: `TsDemux` owns an `Arc<Mutex<OutputArena>>` shared with its `FrameCollector`s, budget-fed through `TsDemuxElement`). Two arenas are deliberately *not* `OutputArena` and say so in a comment: `IpcSink`'s (exists only to pass its fd via SCM_RIGHTS) and `Mp4MuxTransform::flush`'s (one slot, sized to the finished file).
